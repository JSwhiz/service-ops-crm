    ...
    <div className="page-card">
      <div className="section-header" style={{ marginBottom: 12 }}>
        <div>
          ...
          <div className="section-subtitle">Текст, фото и файлы по текущему объекту.</div>
        </div>
      </div>

      <form onSubmit={handleSubmit} style={{ marginBottom: 16 }}>
        <label>
          <div className={styles.fieldLabel} style={{ marginBottom: 6 }}>Новый комментарий</div>
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div key={item.id} className="record-card">
              <div className="page-muted" style={{ marginBottom: 8 }}>{getUserDisplayName(item.createdBy)}</div>
              {item.content ? <div>{item.content}</div> : null}
              <div style={{ marginTop: 10 }}>
                <AttachmentPreviewList files={item.attachments} emptyText="Вложений у комментария нет." />
              </div>
            </div>
          ))
        )}
