...
export function createCandidate(payload: { fullName: string; phone: string; comment?: string; candidateType: CandidateType; objectId?: string | null; managerUserId?: string | null }): Promise<CandidateCard> { return fetcher('/candidates', { method: 'POST', body: JSON.stringify(payload) }); }
export function updateCandidate(id: string, payload: { expectedVersion: number; fullName?: string; phone?: string | null; comment?: string | null; candidateType?: CandidateType; objectId?: string | null }): Promise<CandidateCard> { return fetcher(`/candidates/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }); }
