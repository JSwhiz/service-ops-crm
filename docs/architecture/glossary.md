...
Кандидат — отдельная сущность до найма. `Candidate` не является `Employee` или `User`; назначение менеджера, ответы и SLA хранятся в собственном историческом контуре.

## 22. Notification

Персональное системное уведомление с read-state и ссылкой на исходную сущность. Notification хранится в БД и не является источником истины для бизнес-события.
