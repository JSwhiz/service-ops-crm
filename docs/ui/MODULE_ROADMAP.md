...
Primitives: EntityHeader, Surface/Section, Tabs, DataTable(scroll), FilterBar, Field/FormGrid/FormActions, Dialog, ReasonDialog, Drawer, StatusBadge, Alert/EmptyState/Skeleton. Risk высокий: деньги, partial errors/upload and duplicate mutations. Dependencies Wave0A/0B + Approval shared decision shell из Wave1a; DataTable/Pagination — после первого consumer Inventory. Tabs создаются с первым реализуемым tabbed workspace, затем переиспользуются. Validate GP-EXP-001…003, issuance permissions, closure returns active, linked-order expenses, totals неизменны, uncertain response recovery.
...
- Detail tabs: «Обзор» (catalog + stock), «Движения» (ledger). Deletion state, blockers, deletedAt и restore information показывать в «Обзор» только когда актуальны; Delete/Restore остаются contextual/overflow по current capabilities, hard/soft delete semantics неизменны. Отдельная вкладка «Управление» ради deletion metadata не создаётся; полноценная audit/history capability может обосновать новую вкладку только отдельной задачей. Ledger Дата / Операция / Откуда→куда / Количество / Сумма / Evidence+статус; immutable snapshots не заменять сегодняшней ценой. `/movements` использует тот же renderer/filters, scoped detail сохраняет item scope.
...
Shared primitive API развивается через реальные consumers. Не реализовывать optional capabilities до первого module use-case. Conceptual contract is not a mandate to implement all options in v1.

Wave 0 — только 0A и 0B ниже. Не реализовывать заранее DataTable, Pagination, FilterBar, FilterChip, Tabs, самостоятельный большой Popover или сложный mobile table framework. LinkButton, SearchField, FormGrid/FormActions и прочие контракты вне списка Wave 0 также появляются с первым реальным consumer. Section в 0A только при реальной потребности базовых primitives; Tooltip — KEEP, без расширения positioning/collision/Escape, поздний accessibility polish только при обнаруженной проблеме consumer.

Первые consumers: Approvals — Queue composition, FilterBar, FilterChip, Drawer и ReasonDialog; Inventory — DataTable v1 и Pagination; первый tabbed Entity Workspace — Tabs (при текущем порядке это Inventory detail; если первым реализуется другой workspace, API определяется им). Equipment может расширить DataTable mobile compact list adapter/row actions после подтверждения потребности, а не требовать их заранее. Wave 0B подготавливает минимальную interaction основу под конкретные сценарии Approvals; её использование проверяется в Wave 1a, optional overlay API до других consumers не добавляется. Упоминания Wave0 в module dependencies означают только эту базу; collections/filters/tabs переиспользуются после соответствующего первого consumer.
...
|---|---|---|---|---|---|
| 0A Basic foundation | Existing foundation + token contract | EXTEND Button, IconButton, Surface, Badge/StatusBadge, EmptyState, Skeleton; Alert, Field, PageHeader, EntityHeader; Section только при реальной потребности базовых primitives | CSS cascade, control semantics | Не строить collections/filters/tabs/mobile framework, не менять Tooltip; не массово менять references/legacy palette | Scoped primitive checks, existing shell/login/Objects smoke, focus/pending/error, 390/768/1280/1600px, contrast/zoom |
| 0B Interaction foundation | 0A и конкретные сценарии Approvals | Минимальные Dialog, ConfirmDialog, ReasonDialog, Drawer, OverflowMenu; первый module consumer Drawer/ReasonDialog — Approvals | Focus, nested overlays, pending | Не строить большой standalone Popover/positioning engine и optional API без consumer | Dialog keyboard/focus return/pending/error; интеграционный gate — Approvals в 1a |
| 1a Decision queue | 0A/0B overlays/actions/state | Approvals — первый Queue/FilterBar/FilterChip consumer, интеграция Drawer/ReasonDialog; typed human presentation adapter; минимальный filter popover по необходимости | High stale permission/decision | DTO/payload formats/approval actors | GP-APR, reject/cancel/pending/error, scoped links, no UUID primary |
| 1b Operational finance/resources | 0A/0B + 1a decision shell/filters | Inventory создаёт DataTable v1/Pagination, первый tabbed workspace создаёт Tabs; Equipment расширяет только нужные mobile/actions возможности; Accountability использует готовую основу после Inventory table slice, остальные domain changes могут идти независимо | High money/deletion/evidence | Stock calculations, soft/hard-delete, ledger math, scope ACL | GP-INV/EQP/EXP, version conflicts, histories, current totals, issue photo exceptions, card vs table mobile choices |
...
| 3 Communication and polish | Stable overlays/state from previous waves | Files state polish → Chats visual extraction; Candidates/Counterparties/Absences независимо; Timesheet last in isolated PR; Tooltip accessibility polish только по проблеме реального consumer | Chats/matrix high, registry polish low | Socket/effects/scroll algorithm, finalValue/export/manual capability; no new planner | Two-client chat suite, media states, GP-CAND/TS, keyboard matrix/mobile inspector, archive behavior |
